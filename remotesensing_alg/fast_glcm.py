...
def fast_glcm_std(img, glcm=None, vmin=0, vmax=255, levels=8, ks=5, distance=1.0, angle=0.0):
    '''
    calc glcm std
    '''
    h, w = img.shape
    glcm = fast_glcm(img, vmin, vmax, levels, ks, distance, angle) if glcm is None else glcm
    mean = np.zeros((h, w), dtype=np.float32)
    for i in range(levels):
        for j in range(levels):
            mean += glcm[i, j] * i / (levels) ** 2
...
def fast_glcm_contrast(img, glcm=None, vmin=0, vmax=255, levels=8, ks=5, distance=1.0, angle=0.0):
    '''
    calc glcm contrast
    '''
    h, w = img.shape
...
def fast_glcm_dissimilarity(img, glcm=None, vmin=0, vmax=255, levels=8, ks=5, distance=1.0,
                            angle=0.0):
    '''
...
def fast_glcm_homogeneity(img, glcm=None, vmin=0, vmax=255, levels=8, ks=5, distance=1.0,
                          angle=0.0):
    '''
...
def fast_glcm_ASM(img, glcm=None, vmin=0, vmax=255, levels=8, ks=5, distance=1.0, angle=0.0):
    '''
...
def fast_glcm_max(img, glcm=None, vmin=0, vmax=255, levels=8, ks=5, distance=1.0, angle=0.0):
    '''
    calc glcm max
    '''
    glcm = fast_glcm(img, vmin, vmax, levels, ks, distance, angle) if glcm is None else glcm
...
def fast_glcm_entropy(img, glcm=None, vmin=0, vmax=255, levels=8, ks=5, distance=1.0, angle=0.0):
    '''
    calc glcm entropy
    '''
    glcm = fast_glcm(img, vmin, vmax, levels, ks, distance, angle) if glcm is None else glcm
