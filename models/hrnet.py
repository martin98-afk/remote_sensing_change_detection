...
def _hrnet(arch, num_classes, pretrained, progress, **kwargs):
    from models.hrnet_config import MODEL_CONFIGS
    model = HighResolutionNet(MODEL_CONFIGS[arch], num_classes=num_classes, **kwargs)
    if pretrained:
...
def hrnet18(num_classes, pretrained=True, progress=True, **kwargs):
    r"""HRNet-18 model
    """
    model = _hrnet('hrnet18', num_classes, pretrained, progress,
                   **kwargs)
