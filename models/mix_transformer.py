...
class DWConv(nn.Module):
    ...
    def __init__(self, dim=768):
        ...
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=3, padding=1, stride=1, bias=True,
                                groups=dim)
...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:
            ...
            x_ = self.norm(x_)
            kv = self.kv(x_).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0,
                                                                                            3, 1, 4)
        else:
            kv = self.kv(x).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0,
                                                                                           3, 1, 4)
            # shape: B, N, 2, num_heads, head_dim
            # shape: 2, B, num_heads, N, head_dim
